...
試験を予定する問題については、[Microsoft 認定資格サポート フォーラム](https://trainingsupport.microsoft.com/ja-jp/mcp/forum/mcp_exams-mcp_offer-mcp_eventoffer?sort=LastReplyDate&dir=Desc&tab=All&status=all&mod=&modAge=&advFil=&postedAfter=&postedBefore=&threadType=all&isFilterExpanded=false&page=1) のヘルプを参照してください。フォーラムのモデレーターが 1 営業日以内に返信します。
...
**回答:** このオファーには、Microsoft fundamentals、ロールベースおよび専門認定の試験が含まれます。対象となる認定試験のリストについては、オファー詳細ページの「[対象となる試験](/learn/certifications/microsoft-build-cloud-skills-challenge-2020-free-certification-exam-offer#eligible-exams)」セクションをご覧ください。
